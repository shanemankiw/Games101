...
float insideTriangle_super(float x, float y, const Vector4f* _v)
{   
...
void rst::rasterizer::rasterize_triangle(const Triangle& t, const std::array<Eigen::Vector3f, 3>& view_pos) 
{
        ...
        for (int y = min_y;y < max_y;y++){
            if (insideTriangle(float(x)+0.5, float(y)+0.5, t.v)){
                ...
                if (depth_buf[get_index(x,y)] > z_interpolated){
                    //count_of_points += 1;
                    Eigen::Vector2i point_int;
                    //Eigen::Vector3f color_ref;
                    point_int << x,y;
                    auto interpolated_color = interpolate(alpha, beta, gamma, t.color[0], t.color[1], t.color[2], 1);
                    //auto interpolated_normal= (alpha * t.normal[0] + beta * t.normal[1] + gamma * t.normal[2])/(alpha + beta + gamma);
                    auto interpolated_normal = interpolate(alpha, beta, gamma, t.normal[0], t.normal[1], t.normal[2], 1);
                    auto interpolated_texcoords = interpolate(alpha, beta, gamma, t.tex_coords[0], t.tex_coords[1], t.tex_coords[2], 1);
                    auto interpolated_shadingcoords = (alpha * view_pos[0] + beta * view_pos[1] + gamma * view_pos[2])/(alpha + beta + gamma);
                    fragment_shader_payload payload(interpolated_color, interpolated_normal.normalized(), interpolated_texcoords, texture ? &*texture : nullptr);
                    payload.view_pos = interpolated_shadingcoords;
    
                    auto pixel_color = fragment_shader(payload);
                    set_pixel(point_int, pixel_color);
    ...
    //if (count_of_points > 0) std::cout << count_of_points << std::endl;    
